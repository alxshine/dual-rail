...
With the current implementation state, loading from unbalanced memory causes unbalanced values to be stored in registers.
This fact can be avoided however, by not using globals.
...
It works by working on an intermediate representation (\ir{}) specifically designed to be source and target independent, while allowing for easy automatic optimization.
...
As the optimization passes take \ir{} as input and provide it as output, they can easily be added.
For this reason I implemented the transformation of code into my balanced form as an \llvm{} optimization pass.
