...
mode. The implementation is verified against the test vectors in: National
...
static void AddRoundKey(uint8_t round, uint8_t RoundKey[176], uint8_t *state) {
  uint8_t i, j;
  char buffer[20];
  for (i = 0; i < 4; ++i) {
    for (j = 0; j < 4; ++j) {
...
static void InvMixColumns(uint8_t *state) {
  ...
  for (i = 0; i < 4; ++i) {
    ...
    state[i*4+0] = Multiply(a, 0x0e) ^ Multiply(b, 0x0b) ^ Multiply(c, 0x0d) ^
                     Multiply(d, 0x09);
